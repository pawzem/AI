...
- **Verify two ways — yours, then theirs.** *Pass 1 (on your own):* re-run the self-review checklist — colour grammar + allowed adjacencies — before showing any diagram, and again immediately before every review (Stages 6 and 8); never present one that hasn't just passed it. *Pass 2 (with the user):* never ask a passive "does this look right?" — actively **conduct** the verification together: read flows aloud forward **and** backward, and ask about pivotal events, hotspots, and gaps. Grammar breaks with a one-line edit; the storming is accepted only once the user has walked it with you.
...
**6 — Per-process + joined diagrams.** Produce one mermaid diagram per business process **and** one joined diagram showing cross-process flow. Skeletons in `references/templates.md`. **Re-run the self-review checklist (grammar + adjacencies) on each before presenting** (verification Pass 1) — never show an unvalidated diagram. Add them to the analysis draft, then in the terminal **walk the user through each** — read its primary flow in domain language and call out pivotal events and hotspots, don't just list them — and ask your questions. (The deep two-way validation is Stage 8.) → **Gate:** user has seen all diagrams.
...
**8 — Validate the storming with the user (interactive — verification Pass 2).** This is the heart of the modeling. Don't present it passively; **conduct** the walkthrough per `references/event-storming.md` § *Validation with the user*: read the timeline forward as a narrative, then **backward from the outcome** to surface missing preconditions; confirm pivotal/key events; raise hotspots and opportunities. Ask about **each hotspot** and propose a resolution (Rule of the Game: none may be left dangling), and check the process ends in a stable state and every stakeholder is reasonably happy. After every diagram change, re-run the self-review checklist (Pass 1). → **Gate:** user **accepts** the event storming.
