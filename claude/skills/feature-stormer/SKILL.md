...
- **Re-validate grammar before every review.** Any edit to a diagram → re-run the event-storming self-review checklist (color grammar + the allowed adjacencies). And **always re-run it immediately before asking the user to review** (Stages 6 and 8) — never present a diagram for review that hasn't just passed the checklist. The grammar is easy to break with a one-line edit.
...
**6 — Per-process + joined diagrams.** Produce one mermaid diagram per business process **and** one joined diagram showing cross-process flow. Skeletons in `references/templates.md`. **Re-run the self-review checklist (grammar + adjacencies) on each before presenting** — never show an unvalidated diagram. Add them to the analysis draft, and in the terminal summarize each and ask your questions. → **Gate:** user has seen all diagrams.
